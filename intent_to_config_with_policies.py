            ...
            for all_subnetwork_number, all_interface_name in all_router_data.items():
                if all_as_name == as_name:
                    if all_subnetwork_number[:2] == "SN":
                        ...
                        if any(key.startswith("eBGP_") for key in router_data):
                            config += f"  network {ipv6_address}/64 route-map OWN_ROUTE\n"
                        else:
                    ...
                    if all_router_name != router_name:
                        if all_subnetwork_number[:2]=="LB":
                            ...
                            config += f"  neighbor {ipv6_address} send-community\n"
                elif all_as_name != as_name:
                    if all_subnetwork_number[:4]=="eBGP":
                        for key in router_data.keys():
                            if key.startswith(all_subnetwork_number):
                                ipv6_address = all_as_data['IP_range']['eBGP_interfaces'][all_subnetwork_number]
                                address_neighbor=ipv6_address
                                config += f"  neighbor {ipv6_address} activate\n"
                                config += f"  neighbor {ipv6_address} send-community\n"
    if any(key.startswith("eBGP_") for key in router_data):                          
        for all_neighbor_name, all_neighbor_data in as_data["neighbors"].items():
            if as_name=='AS100':
                AS_neighbor = f"AS{address_neighbor[5]}00"
                if all_neighbor_name == AS_neighbor:
                    if all_neighbor_data == "peer":
                        config += f"  neighbor {ipv6_address} route-map PEER_IN in\n"
                        config += f"  neighbor {ipv6_address} route-map OUT_ROUTE out\n"
                    elif all_neighbor_data == "provider":
                        config += f"  neighbor {ipv6_address} route-map PROVIDER_IN in\n"
                        config += f"  neighbor {ipv6_address} route-map OUT_ROUTE out\n"
            else:
                if all_neighbor_data == "peer":
                        config += f"  neighbor {ipv6_address} route-map PEER_IN in\n"
...
!
route-map OWN_ROUTE permit 10
 set community 4
!
route-map OUT_ROUTE permit 10
 match community 1
 match community 4
!"""
